...
class ShowCatogoryBottomSheet extends StatefulWidget {
  ...
  @override
  State<ShowCatogoryBottomSheet> createState() =>
      _ShowCatogoryBottomSheetState();
}

class _ShowCatogoryBottomSheetState extends State<ShowCatogoryBottomSheet> {
  late final PageController
      _pageController; // for switching  level 2 and level 3 category
  ...
  Widget build(BuildContext context) {
    ...
    return BlocProvider(
      ...
      create: (context) => MainCategoryBloc(widget.purpose)
        ..add(SelectedLevelOneCat(widget.selectedCategory)),
          ...
          if (state is ShowLevelThreeSubCategory) {
            ...
            if (state.navigationMode == SelectedNavigationMode.push) {
              ...
              Future.delayed(const Duration(milliseconds: 800)).then((value) {
                //after navigate to new screen change selected item into unselected
                _selectedL2SubCatName = _selectedL3SubCatName = null;
                context.read<MainCategoryBloc>().add(
                    const DoExtraEvent(eventType: EventAction.refreshPage));
              });
            } else {
              Navigator.pop(context, state.routName);
            }
          } else if (state is ShowLevelTwoSubCategory ||
              state is ComingsoonMainCategory) {
            if (widget.purpose == CategoryBottomSheetPurpose.forSearcinghAd) {
              HomePageScreen.selectMainCategory.value =
                  state.props.first as int;
            }
          }
        },
              ...
              builder: (ctx, cons) => PageView(
                ...
                children: [
                  ...
                  _secondPageLevelThreeCat(
                      cons: cons, state: state, context: context),
                ],
              ),
            ),
          );
          ...
          child: Padding(
            ...
            child: SingleChildScrollView(
              child: Wrap(
                ...
                children:
                    List.from(_levelThreeSubCatList.map((subCat) => InkWell(
                          child: Container(
                            width: cons.maxWidth / 3,
                            height: cons.maxHeight / 3 + 25,
                            padding: const EdgeInsets.symmetric(vertical: 5),
                            foregroundDecoration: (_selectedL3SubCatName ??
                                        '') ==
                                    subCat['cat_name']!
                                ? null
                                : const BoxDecoration(
                          ...
                          onTap: () {
                            ...
                            context.read<MainCategoryBloc>().add(
                                SelectedLevelThreeCat(subCat['root_name']));
                          },
                        ))),
              ),
            ),
          ...
          child: CircularBackButton(
            onPressed: () {
              context
                  ...
                  .add(const DoExtraEvent(eventType: EventAction.goToPageOne));
            },
          ),
        ),
      ],
                        ...
                        children: List.from(
                            _levelTwoSubCatList.map((subCate) => InkWell(
                                  onTap: () {
                                    ...
                                    context
                                        .read<MainCategoryBloc>()
                                        .add(SelectedLevelTwoCat(subCate));
                                  },
                                  child: subCategoryGreyContainer(
                                      cons, subCate['cat_name']),
                                ))),
                      ),
              ),
            ),
  ...
  Container subCategoryGreyContainer(BoxConstraints cons, String subCate) {
    return Container(
      ...
      decoration: BoxDecoration(
        ...
        border: (_selectedL2SubCatName ?? '') == subCate
      ...
      child: Text(
        subCate,
        style: TextStyle(
          color: (_selectedL2SubCatName ?? '') == subCate
              ? kPrimaryColor
              : kDisabledText,
          fontSize: 13,
