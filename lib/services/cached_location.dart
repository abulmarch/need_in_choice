...
class CacheLocation {
  ...
  saveAddressLocally(CachedLocationData cachedLocationData){
    try {
      ...
      List<Map<String, dynamic>> savingData = predictions.map((e) => e.toJson()).toList();
  ...
  _getAddressFromLocally(){
    ...
    if(addressList != null){
      try {
        ...
        predictions = parsed.map((cachedData) => CachedLocationData.fromJson(cachedData)).toList();
      } catch (e) {
  ...
  Future<bool> finadPlaceDetails({required AutocompletePrediction prediction, required BuildContext context}) async {//https://maps.googleapis.com/maps/api/place/details/json
    Uri uri = Uri.https(
      'maps.googleapis.com',
    ...
    if (response != null) {
      ...
      final cachedLocationData = CachedLocationData(prediction: prediction,placeDetails: result);
  ...
  saveDataToUserProfile(CachedLocationData cachedLocationData, BuildContext context) async {
...
class PlaceDetails {
  ...
  factory PlaceDetails.fromJson(Map<String, dynamic> json) {
    ...
    log(json.toString());
      ...
      final addressComponents = result.firstWhere((element) => pinCodeRegex.hasMatch(element['long_name']));
